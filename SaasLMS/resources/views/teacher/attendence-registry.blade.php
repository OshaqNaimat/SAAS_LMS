            ...
            <div class="flex flex-col sm:flex-row sm:items-center sm:justify-between gap-4 mb-8">
                ...
                <div class="flex flex-wrap gap-2">
                    ...
                    <div
                        ...
                        <span class="text-xs font-bold text-white">{{ now()->format('d F Y') }}</span>
                    </div>
                </div>
            </div>
                            ...
                            <div
                                ...
                                <div class="flex items-center gap-2">
                                    <button type="button" onclick="setStatus({{ $student->id }}, 'present', this)"
                                        class="px-3 py-1.5 rounded-lg text-xs font-{{ $currentStatus === 'present' ? 'bold' : 'semibold' }} transition {{ $currentStatus === 'present' ? 'bg-emerald-500 text-white shadow-md shadow-emerald-500/20 border border-emerald-400/20' : 'bg-slate-900 text-gray-400 hover:text-white border border-slate-800' }}">Present</button>
                                    <button type="button" onclick="setStatus({{ $student->id }}, 'absent', this)"
                                        class="px-3 py-1.5 rounded-lg text-xs font-{{ $currentStatus === 'absent' ? 'bold' : 'semibold' }} transition {{ $currentStatus === 'absent' ? 'bg-rose-500 text-white shadow-md shadow-rose-500/20 border border-rose-400/20' : 'bg-slate-900 text-gray-400 hover:text-white border border-slate-800' }}">Absent</button>
                                    <button type="button"
                                        onclick="setStatus({{ $student->id }}, 'approved_leave', this)"
                                        class="px-3 py-1.5 rounded-lg text-xs font-{{ $currentStatus === 'approved_leave' ? 'bold' : 'semibold' }} transition {{ $currentStatus === 'approved_leave' ? 'bg-amber-500 text-white shadow-md shadow-amber-500/20 border border-amber-400/20' : 'bg-slate-900 text-gray-400 hover:text-white border border-slate-800' }}">Leave</button>
                                </div>
                            </div>
                        @empty
                            <p class="text-center text-gray-500 text-sm py-8">No students found in this class.</p>
                        @endforelse
                    ...
                    <div class="bg-[#111c2a] border border-slate-800 rounded-2xl p-5">
                        ...
                        <div class="grid grid-cols-3 gap-2">
                            <div class="bg-emerald-500/5 border border-emerald-500/20 rounded-xl p-3 text-center">
                                ...
                                <span class="text-lg font-bold text-white mt-1 block">{{ $presentCount }}</span>
                            </div>
                            <div class="bg-rose-500/5 border border-rose-500/20 rounded-xl p-3 text-center">
                                ...
                                <span class="text-lg font-bold text-white mt-1 block">{{ $absentCount }}</span>
                            </div>
                            <div class="bg-amber-500/5 border border-amber-500/20 rounded-xl p-3 text-center">
                                ...
                                <span class="text-lg font-bold text-white mt-1 block">{{ $leaveCount }}</span>
                            </div>
                        </div>
                        <div class="mt-4 pt-4 border-t border-slate-800/60 flex justify-between text-xs text-gray-400">
                            ...
                            <span class="font-bold text-white">{{ $students->count() }} Students</span>
                        </div>
                    </div>
